...
export function getCurrentEventPriority(): * {
  console.groupCollapsed('获取当前事件的优先级')
  console.log(`当事件触发更新后，react会去获取当前优先级，这里分为三种情况
    1:react事件。在初始应用时候react会绑定所有浏览器事件到root上，绑定的事件listener是一个带有优先级的函数，所以react事件触发更新会根据这个listener获取到对应优先级，（如果是react事件是不会触发当前log）
